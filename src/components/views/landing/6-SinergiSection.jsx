        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 w-full mt-4">
          {networks.map((item, idx) => (
            <FadeUp key={idx} delay={0.2 + (idx * 0.1)} className="flex flex-col items-center gap-4 text-center">
              <div className="w-32 h-32 bg-blue-900 rounded-full flex items-center justify-center overflow-hidden shrink-0 shadow-md">
                <img 
                  src={item.logoUrl || item.image || "https://placehold.co/128x128"} 
                  alt={`Logo ${item.name}`} 
