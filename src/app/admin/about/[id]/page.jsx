...
export default function EditAboutPage() {
  ...
  const [form, setForm] = useState({ title: "", subtitle: "", content: "", vision: "", mission: "", imageUrl: "" });
  ...
  useEffect(() => {
    const fetchAbout = async () => {
      ...
      if (data.about) {
        const a = data.about;
        setForm({ title: a.title || "", subtitle: a.subtitle || "", content: a.content || "", vision: a.vision || "", mission: a.mission || "", imageUrl: a.imageUrl || "" });
      }
      setLoading(false);
    };
    fetchAbout();
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/about/${params.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Update about page content</p>
        </div>
      </div>
